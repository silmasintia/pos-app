...
function loadUserEdit(userId) {
    $.get(`/users/${userId}/edit`, function (response) {
       ...
       if (response.id === currentUserId) {
            $("#edit_role_id")
                .prop("disabled", true)
                .attr("title", "You cannot change your own role");
        } else {
            $("#edit_role_id")
                .prop("disabled", false)
                .removeAttr("title");
        ...
        $("#edit_status_display_label").text(
            response.status_display ? "Public" : "Private"
        );
...
function submitUserForm(form, url, method, defaultText, modalId) {
    ...
    if (method === "PUT") {
        ...
        if (removeImageCheckbox.length) {
            removeImageCheckbox.val(
                removeImageCheckbox.is(":checked") ? "1" : "0"
            );
        }

        if (removeBannerCheckbox.length) {
            removeBannerCheckbox.val(
                removeBannerCheckbox.is(":checked") ? "1" : "0"
            );
        }
    }
